 ...
 *     화면을 본다. 저장된 값은 그대로다.
 * 앞의 둘을 갈라 둔 것은 알림 설정 화면에서 온 구분이고, 그 구분을 그대로
 * 따른다 — 두 단추의 뜻이 섞이면 "되돌렸는데 왜 그대로냐"가 된다.
 * 🔴 그 화면은 2026-09-22 에 이 저장소를 떠나 통합 로그인 포털로 갔다
 * (`dss-auth` 의 `/admin/notifications` — `src/app/admin/notifications/page.tsx`).
 * 같은 「전부 기본값으로」가 그날 그 화면에도 들어갔으니, 두 단추의 뜻을 따라갈
 * 곳은 이제 거기다(이 저장소에는 `NotificationSettings.tsx` 가 없다).
 *
 * ── 바뀐 것만 보낸다 ────────────────────────────────────────────────────
