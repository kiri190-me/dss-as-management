 ...
 * `엑셀` 은 내려받기 라우트로 간다. 그런데 줄 전체를 `<Link>` 로 감싸고 그 안에
      ...
      <ul className="flex flex-col gap-2">
        {rows.map((row) => (
          ...
          >
            ...
            <span className="relative z-10 flex flex-wrap items-center gap-x-3 gap-y-1">
              <Link
                href={row.printHref}
                aria-label={`${row.kindLabel} ${row.name} 미리보기`}
                className="whitespace-nowrap text-xs font-medium text-sky-700 underline dark:text-sky-400"
              >
                미리보기
              </Link>
