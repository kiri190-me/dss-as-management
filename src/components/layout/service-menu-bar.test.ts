...
test("🔴 생김새를 부르는 줄이 있고, 노치 인셋 파일은 **없다**", () => {
  ...
  // 🔴 종의 생김새는 **다른 파일**이다. 묶음은 조각마다 스타일시트를 한 장씩
  // 따로 내주므로(CSS 에서 @import 로 묶는 것을 그쪽 시험이 막는다) 둘 다
  // 있어야 한다. 한 장만 부르면 나머지 조각이 아무 오류 없이 모양만 잃는다.
  assert.ok(
    appLayout.includes('import "@dss/ui/notification-bell.css";'),
    "@dss/ui 의 종 CSS 를 부르지 않는다 — 알림 종이 모양 없이 뜬다"
  );
