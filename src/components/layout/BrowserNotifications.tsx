...
export default function BrowserNotifications({
   ...
   * 안 보일 때·글자를 치는 중일 때 쉬는 규칙도, 연달아 두드리지 않게 하는 최소
   * 간격도 거기 있다. 여기는 계기와 시각을 넘기고 결과대로 부르기만 한다.
   */
   ...
   * 🔴 화면으로 돌아오는 순간 즉시 한 번.
   *
   * 폰은 다른 앱으로 수시로 나갔다 들어온다. 나가 있는 동안은 화면이 안 보여
   * 쉬는데(shouldRefreshNotifications), 그것만 있으면 **돌아와도 다음 주기까지
   * 최대 1분** 낡은 값을 본다 — "새로고침을 해야만 알림이 바뀐다"는 신고의
   * 정체가 그 1분이다. 주기를 짧게 하는 대신 돌아온 그 순간에 한 번 센다.
   *
