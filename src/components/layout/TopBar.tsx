...
export default function TopBar({ title, onMenuClick, notifications = [] }: TopBarProps) {
  return (
    ...
    //
    // 그 `pt-[env(safe-area-inset-top)]` 자체는 이제 여기 없다 — 맨 위에
    // 서비스 메뉴바가 앉으면서 AppShell 로 옮겼다. 띠가 없을 때는 AppShell 이
    // 이 <header> 에 같은 패딩을 그대로 걸어 준다(`[&>header]:pt-…`), 띠가
    // 있을 때는 띠가 가진다. 둘 다 두면 아이폰에서 두 번 밀린다.
    <header className="flex min-h-14 shrink-0 items-center gap-3 border-b border-zinc-200 bg-white px-4 dark:border-zinc-800 dark:bg-zinc-900">
      <button
        type="button"
        onClick={onMenuClick}
        aria-label="메뉴 열기"
