...
import { ServiceMenuBar, type ServiceMenuEntry } from "@dss/ui";
...
export default function AppShell({ children, user, accessibleAreaKeys, canEnterDeveloperMode, myPendingApprovalCount = 0, notifications = [], portalInbox, portalUrl = null, services = [], currentServiceId = null }: AppShellProps) {
  ...
  return (
    ...
    <div className="flex min-h-0 flex-1 flex-col">
      {/*
        머리말. 노치 인셋(env(safe-area-inset-top))은 **맨 위 요소 하나만**
        가지는데, 서비스 메뉴바가 이 머리말 **안**으로 들어오면서 맨 위 요소는
        다시 <header> 자신이 되었다 — 그래서 그 패딩은 TopBar 로 돌아갔고,
        여기서 거는 것은 아무것도 없다(한때 띠가 맨 위였을 때만 이 자리에
        `[&>header]:pt-…` 가 있었다).
      */}
        ...
        <TopBar
          ...
          serviceMenu={
            /*
              ...
              사이로 앉는다(variant="inline") — 위에 회색 띠로 따로 앉히면
              화면 맨 위가 두 층이 되어 답답하고 본문이 한 줄만큼 줄어든다는
              사용자 지적(2026-09-18) 때문이다. 그 모습은 바탕도 아래 테두리도
              없이 머리말 위에 그대로 얹히고, 지금 있는 서비스는 굵기·글자색·
              2px 밑줄로 알린다(회색 띠일 때의 「흰 칸으로 띄우기」는 흰 머리말
              위에서 보이지 않는다).
