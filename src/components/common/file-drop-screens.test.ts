...
const SITES: {
  ...
  {
    file: "src/components/settings/ImprovementRequestScreenshots.tsx",
    zone: 'name="improvement-request-screenshots"',
    multiple: true,
    drop: "onFiles={onAddFiles}",
    picker: "<ScreenshotAddButton onFiles={onAddFiles}",
    why: "개선요청 스크린샷 — 판정은 부르는 쪽의 screenScreenshotBatch",
  },
  {
...
describe("🔴 공통 조각은 검사를 하지 않는다", () => {
  test("허용목록 · 자리마다의 판정 모듈을 부르지 않는다", () => {
    ...
    for (const forbidden of [
      ...
      "attachment-category",
      "improvement-request-screenshot-files",
...
describe("기존 고르기 칸은 그대로다", () => {
  test("여덟 자리 모두 고르기 칸(또는 고르기 단추)이 남아 있다", () => {
    ...
    const withPicker = [
      ...
      "src/components/settings/ImprovementRequestScreenshots.tsx",
