 ...
 *
 * ── 🔴 화면이 보내는 것은 둘뿐이다 ───────────────────────────────────
 * `file`(연락서 원본) 과 `repairCaseId`(사람이 고른 수리 건). 미리보기 결과 ·
 * 넣을 줄 · 부품 목록은 **받지 않는다** — 저장 함수가 파일에서 처음부터 다시
 * 만든다(services/kyosan-report-import.ts 머리말). 화면이 계산한 것을 받는 순간
 * 「사람이 본 것」과 「들어간 것」이 갈라진다.
 *
 * ── 🔴 로그에 입력값을 싣지 않는다 ───────────────────────────────────
