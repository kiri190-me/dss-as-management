...
test("🔴 대표 지정 판정을 화면이 직접 계산하지 않는다 — 서버 페이지가 내려보낸다", () => {
  ...
  // 🔴 아래 화면들의 prop 이름도 2026-09-07 에 `canManageRepresentatives` 로
  // 바뀌었다. 이름만 고친 순수 개명이지만 여기서 대조하는 문자열이 그것이라
  // 함께 옮긴다 — 그리고 **모든 자리**에 넘기는지 개수로 확인한다. 한 곳이라도
  // 빠지면 목록은 열리는데 위임 칸이 잠긴다(또는 그 반대).
  //
  // 2026-09-09: 「출하 승인 절차」(결재선) 탭이 셋째 소비자로 들어와 2 → 3 이
  // 됐다. 같은 값을 쓰는 이유는 **절차가 「출하 대표」를 대신하기로 해서**다 —
  // 새 권한 영역을 만들면 절차를 고칠 수 있는 사람과 대표를 지정할 수 있는
  // 사람이 갈리는데, 둘은 「누가 최종 출하 승인을 하는가」라는 한 가지 물음의
  // 두 얼굴이라 갈려서는 안 된다(그리고 이미 저장된 역할별 권한 설정이 새
  // 영역만 비어 있는 채로 남는다).
  //
  // 🔴 개수를 「2 이상」 같은 바닥선으로 느슨하게 두지 않는다. 정확한 개수인
  // 것이 이 단언의 값 그 자체다 — 새 소비자가 생기면 여기서 걸려 사람이 「그
  // 자리도 같은 열쇠를 쓰는 것이 맞는가」를 한 번 보게 된다. 실제로 위 탭이
  // 들어올 때 그렇게 걸렸다.
  assert.equal(
    [...screen.matchAll(/canManageRepresentatives=\{canManageRepresentatives\}/g)].length,
    3,
    "받은 값을 아래 세 화면(대표 목록·위임·출하 승인 절차) 모두에 넘기지 않는다"
  );
...
test("🔴 화면 페이지와 서버 mutation·액션 다섯 파일이 같은 영역 열쇠·같은 수준을 쓴다", () => {
  // 열쇠나 수준이 한 곳만 달라지면 개발자에게 또 갈린다 — 그때 증상은
  // 「단추는 보이는데 저장은 거절」이고, 화면에는 아무 설명도 남지 않는다.
  //
  // 🔴 2026-09-09: 아래 목록이 **판정하는 파일 전부**여야 한다. 「출하 승인
  // 절차」가 들어올 때 같은 열쇠를 쓰는 자리가 둘 늘었는데 이 목록에 없어서,
  // 새 자리 둘이 조용히 안 지켜지고 있었다 — 통과하니까 저절로 알려 주지도
  // 않는다. 같은 열쇠로 판정하는 자리를 새로 만들면 **여기에도 넣을 것.**
  const sources = [
    ...
    "src/lib/server/actions/shipment-approval-routes.ts",
  ];
  ...
  // 화면 1 + 대표 지정 1 + 위임 생성·철회 2 + 절차 저장(mutation) 1 +
  // 절차 저장(서버 액션) 1 = 6. 늘거나 줄면 새 자리가 생긴 것이므로 같은
  // 열쇠·수준인지 사람이 한 번 봐야 한다.
  //
  // 🔴 위 정규식은 **주석과 코드를 구별하지 못한다.** 어느 파일이든 주석에
  // `hasPermission(actor, "…", "…")` 을 호출 모양 그대로 적어 두면 그 문장이
  // 여기서 한 번 더 세어지고, 그러면 **주석을 고치거나 지우기만 해도 이 시험이
  // 깨진다** — 덫이 진짜 신호가 아니라 잡음이 된다. 설명이 필요하면 호출 모양이
  // 아니라 문장으로 적을 것(mutations/shipment-approval-routes.ts 머리말이
  // 그렇게 적혀 있고, 그 이유도 거기 적혀 있다).
  assert.equal(total, 6, `대표 지정 판정 자리의 수가 달라졌다: ${total}`);
});
