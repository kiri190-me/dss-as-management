...
/**
 * 화면 토큰(색·모서리·글자 크기)을 바꿀 수 있는가 — 개발자 모드 관문과 같은 넓이.
 *
 * ── 왜 이름을 따로 두면서 답은 빌려 오는가 ──────────────────────────────
 * notification-settings-authorization.ts 가 canManageRolePermissions 에 답을
 * 빌리면서도 이름을 따로 둔 것과 정확히 같은 모양이다. 이것은 "누가 개발자
 * 화면에 들어가는가"와 **다른 질문**이다 — "누가 앱 전체의 색과 글자 크기를
 * 정하는가"다. 지금은 두 답이 같아야 하지만(편집기가 개발자 모드 화면 안에
 * 있다) 질문마다 이름이 있어야, 나중에 한쪽만 바뀔 때 고칠 자리가 어디인지
 * 알 수 있다. 그날 고칠 것은 이 함수 한 줄이다.
 *
 ...
 * 최고관리자급으로 올리는 스위치 그 자체라서 특별히 좁혀 둔 것이다. 화면 토큰은
 * 권한을 하나도 움직이지 않는다 — 색과 모서리와 글자 크기가 바뀔 뿐이다.
 * 여기서 같이 좁히면 **"들어와서 편집기는 보이는데 누르면 거절"** 이 된다.
 *
 * ── 🔴 actorMay 로 감싸지 않는다 ────────────────────────────────────────
 ...
 */
export function mayEditUiTheme(actor: DeveloperModeActor<string>): boolean {
