...
import { guardImprovementRequestAttachmentChange, guardQuoteAttachmentChange } from "./attachments";
 ...
 *
 * ── 넷째 주인 — 견적서의 결재 PDF · 수기 엑셀 (2026-09-15 Q2) ─────────────
...
async function loadForTrash(tx: Tx, attachmentId: string) {
  const [row] = await tx
    .select({
      ...
      // 견적서 파일이면 견적서 행을 잠그고 휴지통 · 칸을 본다(파일 헤더의 '넷째 주인').
...
/**
 * 견적서 파일이면 견적서 행을 잠그고 판정한다(파일 헤더의 '넷째 주인'). 다른 주인이면
...
export async function softDeleteAttachment(params: {
  ...
  return db.transaction(async (tx): Promise<AttachmentTrashResult> => {
    ...
    const rejected =
      (await guardImprovementRequestOwner(tx, current, {
        actorUserId: params.actorUserId,
        canManageImprovementRequests: params.canManageImprovementRequests,
        adding: false,
      })) ?? (await guardQuoteOwner(tx, current, { adding: false }));
 ...
 * 헤더의 '셋째 주인'). 견적서 파일이면 견적서가 휴지통이 아니어야 하고 **그 칸이
 * 비어 있어야** 한다(파일 헤더의 '넷째 주인').
 */
  ...
  return db.transaction(async (tx): Promise<AttachmentTrashResult> => {
    ...
    const rejected =
      (await guardImprovementRequestOwner(tx, current, {
        ...
        canManageImprovementRequests: params.canManageImprovementRequests,
        adding: true,
      })) ?? (await guardQuoteOwner(tx, current, { adding: true }));
    if (rejected) return rejected;
 ...
 * 개선 요청 쪽(trashAttachmentsOfDeletedImprovementRequest)과 같은 모양이다: 소프트 삭제
