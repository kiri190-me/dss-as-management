...
import { UNIT_PRICE_FIELD_ERROR_KEY } from "@/lib/validation/part-unit-price-input";
 ...
 * 단가 저장 — 통합 시험
 * ============================================================================
 * 🔴 **부품 하나에 단가 하나다**(2026-09-17 사용자 정정 —
 * schema/part-unit-prices.ts 머리말). 소유구분 축이 없어졌다. 한계수량은
 * **그대로 소유구분별**이고, 이 파일은 그 둘이 한 트랜잭션에 들어가는 것도 함께
 * 본다.
 *
 * 여기서 못 박는 것은 다섯이다.
 *
 ...
 *     않는다 — 한 구역에서 한 단추로 저장하므로 반쪽 상태가 있으면 안 된다.
 *  3. **바뀌지 않은 칸은 쓰지 않는다.** "125000" 과 "125000.00" 은 DB 에서
 *     같은 값이다 — 글자로 대면 매번 다르다고 나와 updated_by 가 갈아엎어진다.
 *  4. **오류 키가 겹치지 않는다.** 단가 오류는 `unitPrice` 키로 온다 — 한계수량
 *     오류(소유자 코드)와 섞이면 엉뚱한 칸에 빨간 글씨가 붙는다.
 *  5. **🔴 부품마다 줄이 하나뿐이다.** 두 번 저장해도 줄이 늘지 않는다.
 *
...
/**
 * 지금 저장돼 있는 단가 줄 — **0줄 아니면 1줄**이다. 정하지 않은 부품은 줄이
 * 아예 없어야 한다(그것이 "0원(무상)"과 갈라지는 자리다).
 */
async function storedPrices(partId: string) {
  return db
    .select({
      id: partUnitPrices.id,
...
function savePrice(partId: string, unitPrice: string, actorUserId: string) {
  return savePartOwnerSettings({ partId, entries: [], unitPrice, actorUserId });
}
...
describe("단가 저장", () => {
  test("값을 적으면 그대로 들어가고, 적은 사람이 기록된다", async () => {
    ...
    const result = await savePrice(partId, "1,250,000.50", superAdminId);
    assert.equal(result.ok, true, JSON.stringify(result));

    const rows = await storedPrices(partId);
    assert.equal(rows.length, 1);
    assert.equal(rows[0].unitPrice, "1250000.50", "쉼표는 지워지고 numeric(15,2) 로 들어간다");
    assert.equal(rows[0].updatedBy, superAdminId);
  });

  test("🔴 부품마다 줄이 하나다 — 다시 저장해도 줄이 늘지 않는다", async () => {
    const partId = await createTestPart();
    await savePrice(partId, "100000", superAdminId);
    const [created] = await storedPrices(partId);

    await savePrice(partId, "150000", inventoryManagerId);

    const rows = await storedPrices(partId);
    assert.equal(rows.length, 1, "소유구분 축이 없으므로 줄은 언제나 하나다");
    assert.equal(rows[0].id, created.id, "같은 줄이 갱신돼야 한다");
    assert.equal(Number(rows[0].unitPrice), 150000);
    assert.equal(rows[0].updatedBy, inventoryManagerId);
  });

  test("🔴 빈 칸은 0 으로 저장하지 않고 줄을 지운다 — 견적서가 0원으로 청구하면 안 된다", async () => {
    const partId = await createTestPart();
    await savePrice(partId, "125000", superAdminId);
    assert.equal((await storedPrices(partId)).length, 1);

    await savePrice(partId, "", superAdminId);
    assert.deepEqual(await storedPrices(partId), [], "빈 칸은 줄이 사라져야 한다");
  });

  test("0 은 '무상 부품'으로 저장된다 — 빈 칸과 다르다", async () => {
    const partId = await createTestPart();
    await savePrice(partId, "0", superAdminId);
  ...
  test("바뀌지 않은 칸은 쓰지 않는다 — '125000'과 '125000.00'은 같은 값이다", async () => {
    ...
    const result = await savePrice(partId, "125000.00", inventoryManagerId);
  ...
  test("음수는 DB 가 아니라 검증에서 막힌다 — 사용자에게 이유가 보인다", async () => {
    const partId = await createTestPart();
    const result = await savePrice(partId, "-1", superAdminId);
    assert.equal(result.ok, false);
    if (result.ok) return;
    assert.equal(result.code, "INVALID_INPUT");
    assert.ok(result.fieldErrors?.[UNIT_PRICE_FIELD_ERROR_KEY], "단가 칸 키로 와야 한다");
    // 한계수량 칸 키로는 오지 않는다 — 그러면 엉뚱한 칸에 빨간 글씨가 붙는다.
    assert.ok(!result.fieldErrors?.DSS);
  });

  test("🔴 한계수량은 소유구분별 그대로다 — 단가만 부품당 하나가 됐다", async () => {
    const partId = await createTestPart();
    const result = await savePartOwnerSettings({
      partId,
      entries: [
        { owner: "DSS", minimumQuantity: "20" },
        { owner: "TEST", minimumQuantity: "2" },
      ],
      unitPrice: "125000",
      actorUserId: superAdminId,
    });
    ...
    assert.deepEqual(
      (await storedMinimums(partId)).map((r) => [r.owner, r.minimumQuantity]).sort(),
      [
        ["DSS", 20],
        ["TEST", 2],
      ],
      "한계수량은 소유자마다 따로 남아야 한다"
    );
    assert.equal((await storedPrices(partId)).length, 1, "단가는 한 줄뿐이다");
  });
});
...
describe("한계수량과 한 트랜잭션", () => {
  ...
  test("둘 다 맞으면 둘 다 저장된다", async () => {
    ...
    assert.deepEqual(
      (await storedPrices(partId)).map((r) => Number(r.unitPrice)),
      [125000]
    );
  });

  test("🔴 단가를 안 보내면(undefined) 단가는 건드리지 않는다 — 빈 문자열과 다르다", async () => {
    const partId = await createTestPart();
    await savePrice(partId, "125000", superAdminId);
    ...
    const result = await savePartOwnerSettings({
      ...
      // unitPrice 를 주지 않는다. 빈 문자열이었다면 줄이 지워졌어야 한다.
      actorUserId: superAdminId,
    });
    assert.equal(result.ok, true);
  ...
  test("없는 부품은 NOT_FOUND", async () => {
    const result = await savePrice(randomUUID(), "1", superAdminId);
    assert.equal(result.ok, false);
    if (result.ok) return;
...
describe("O/H 단가 저장", () => {
  ...
  test("🔴 O/H 단가를 넣어도 일반 단가는 그대로다 — 두 표는 서로 간섭하지 않는다", async () => {
    ...
    assert.deepEqual(
      (await storedPrices(partId)).map((row) => Number(row.unitPrice)),
      [100000],
      "일반 단가가 달라지면 안 된다"
    );
    ...
    await savePrice(partId, "150000", superAdminId);
    assert.equal(
