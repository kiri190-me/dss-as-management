...
import {
  ...
  parseUnitPriceValue,
  validatePartUnitPrice,
} from "./part-unit-price-input";
...
test("🔴 값 하나를 받는다 — 소유구분을 보지 않는다", () => {
  // 2026-09-17 사용자 정정: 소유구분에 따라 부품의 단가가 달라지지 않는다.
  // 소유자 넷을 배열로 받던 검증이 값 하나 검증으로 바뀐 자리다.
  const result = validatePartUnitPrice("1,000.50");
  assert.ok(result.ok);
  assert.equal(result.unitPrice, "1000.50");
});

test("🔴 빈 칸은 null 이다 — 저장 쪽이 그 줄을 지운다(0 으로 저장하지 않는다)", () => {
  for (const empty of ["", "   ", null, undefined]) {
    const result = validatePartUnitPrice(empty);
    assert.ok(result.ok, String(empty));
    assert.equal(result.unitPrice, null, String(empty));
  }
});

test("0 은 '무상 부품'이라 그대로 남는다 — 빈 칸과 다르다", () => {
  const result = validatePartUnitPrice("0");
  assert.ok(result.ok);
  assert.equal(result.unitPrice, "0", "null 이 되면 '정하지 않음'과 구별되지 않는다");
});

test("틀린 값은 단가 칸 키로 거절한다 — 한계수량 칸에 빨간 글씨가 뜨면 안 된다", () => {
  const result = validatePartUnitPrice("-5");
  assert.equal(result.ok, false);
  if (result.ok) return;
  assert.match(result.fieldErrors[UNIT_PRICE_FIELD_ERROR_KEY], /단가/);
  // 한계수량 검증이 쓰는 키(소유자 코드·`owner`)와 겹치지 않는다.
  assert.ok(!result.fieldErrors.DSS);
  assert.ok(!result.fieldErrors.owner);
});

test("값으로 읽을 수 없는 것은 거절한다", () => {
  assert.equal(validatePartUnitPrice({ owner: "DSS" }).ok, false);
  assert.equal(validatePartUnitPrice(["125000"]).ok, false);
});
