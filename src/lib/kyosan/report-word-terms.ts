...
export const KYOSAN_WORD_TERMS: Readonly<Record<string, string>> = Object.freeze({
  ...
  "基板": "기판",
  /**
   * 🔴 **사용 부품 표 실측(2026-09-23)에서 1줄**(접수 `D210102`). 이식된 7줄 가운데
   * 사전이 못 읽던 유일한 줄이었다.
   *
   * 🔴 **왜 아래 낱말 `通信` 만으로는 모자랐나** — `基板`(기판)이 이미 있어
   * `通信` + `基板` 이 조립되기는 한다. 그런데 나오는 글자가 `통신기판` 이라
   * **띄어쓰기가 안 맞는다.** 낱말 값에 꼬리 공백을 붙여 고치는 길은 막혀 있다
   * (아래 `通信` 의 ⚠️ 참조 — 홀로 있는 줄에 공백이 남아 통계 열쇠가 갈라진다).
   *
   * 그래서 **긴 짝을 함께 둔다.** 후보는 **길이 내림차순**이라 4자인 이 줄이 2자인
   * `通信` 보다 **먼저 걸린다**(최장 일치 — 이 파일 머리말의 그 규칙이 그대로
   * 일한다). `終段AMP入力保護用ヒューズ` 가 `ヒューズ` 를 이기는 것과 같은 자리다.
   */
  "通信基板": "통신 기판",
  /**
   * 위 줄의 낱말 짝. 🔴 이 사전의 규칙대로 **통째만 넣지 않고 낱말도 넣는다** —
   * 「실제로 들어온 자료에서 본 낱말만 넣는다」를 지키면서 `通信部品` 처럼 다른
   * 조합도 덮는다. 자유 기술 사전의 `通信異常`(통신 이상)은 이것보다 길어 **먼저**
   * 걸리므로 그 줄의 답은 바뀌지 않는다.
   *
   * ⚠️ 값에 공백을 두지 않는다. 뒤에 공백을 붙이면 `通信` 홀로 있는 줄이
   * `"통신 "` 로 끝나 사용 부품 칸에 **꼬리 공백**이 저장되고, 그 글자가 통계의
   * 묶는 열쇠라 `통신` 과 다른 조각이 된다. 띄어쓰기가 필요한 조합은 위처럼
   * **긴 짝을 따로 적어** 해결한다.
   */
  "通信": "통신",
  "部品": "부품",
