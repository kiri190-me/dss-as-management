 ...
 *     못한다.
 *  2. 🔴 **일부러 뺀 두 표가 들어와 있지 않다.** 값으로 단언한다 — 나중에 누가
 *     "빠졌네" 하며 넣으면 이 시험이 먼저 걸린다.
 *  3. 등록부의 기본 문구가 **자기 검증기를 통과한다.** 통과하지 못하는 기본값이
 *     하나라도 있으면 "되돌리기"가 그 자리에서만 실패한다.
 *  4. 병합은 **너그럽다** — 등록부에 없는 키의 남은 행 하나 때문에 화면이 죽으면
 *     안 된다.
 * ============================================================================
 */
...
test("🔴 묶음이 8개이고 문구가 모두 46개다", () => {
  assert.equal(UI_TEXT_GROUPS.length, 8);
...
test("🔴 일부러 뺀 두 표는 등록부에 없다 — 문구 값으로 단언한다", () => {
