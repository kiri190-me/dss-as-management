...
import { getRepairCaseTrashRetentionStatus, REPAIR_CASE_TRASH_RETENTION_DAYS } from "./repair-case-trash-retention";

test("at the moment of deletion, 15 whole days remain and it is not expired", () => {
  const deletedAt = "2026-08-01T00:00:00.000Z";
  const status = getRepairCaseTrashRetentionStatus(deletedAt, new Date(deletedAt));
  assert.equal(status.daysRemaining, REPAIR_CASE_TRASH_RETENTION_DAYS);
...
test("14 days after deletion, 1 day remains", () => {
  ...
  const status = getRepairCaseTrashRetentionStatus(deletedAt, now);
