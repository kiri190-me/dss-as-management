 ...
 *                            →  improvement-requests/{개선요청id}/{첨부id}.{확장자}
 *                            →  quotes/{견적서id}/{첨부id}.{확장자}
 *
 * ── 첫 마디가 주인을 말한다 ──────────────────────────────────────────────
 * 첨부의 주인은 접수 건 · 제품 모델 · 개선 요청 · 견적서 중 하나이고
 * (schema/attachments.ts의 attachments_owner_not_both ·
 * attachments_improvement_owner_alone · attachments_quote_owner_alone 참조),
 * stored_path의 **첫 마디가 그 넷을 가른다.** 그래서 폴더 하나만 보면 그것이
 * 어느 쪽의 파일인지 알 수 있고, 백업이 DB 없이 디스크만 훑어도 구조가 읽힌다.
 * 네 접두어는 아래 네 상수가 정본이며, assertPortableStoredPath가 그 넷
 * **말고는 아무것도 받지 않는다.**
 *
...
/** 개선 요청 글에 붙는 첨부(스크린샷)의 첫 마디. 위 둘과 같은 규칙이다. */
export const ATTACHMENT_IMPROVEMENT_REQUEST_STORED_PATH_PREFIX = "improvement-requests";
...
/**
 * ============================================================================
 * 개선 요청 첨부 — 세 번째 벌 (2026-09-13)
 * ============================================================================
 * 설정 › 개선 요청 글에 붙는 스크린샷의 자리다. 제품 모델 벌을 더할 때와 같은
 * 까닭으로 **위 두 벌을 건드리지 않고 나란히 새로 둔다** — 서명도 본문도 그대로다.
 *
 * 검사 규칙은 위 두 벌과 **글자 하나까지 같다** — UUID 형식 확인, 확장자 허용목록,
 * 소문자 눕히기, path.join 금지. 다른 것은 첫 마디뿐이다. 스크린샷을 이미지로만
 * 좁히는 것은 여기가 아니라 분류 허용목록(CATEGORY_EXTENSION_ALLOWLIST의
 * SCREENSHOT)이 맡는다 — 경로 함수는 "옮길 수 있는 경로인가"만 본다.
 * ============================================================================
 */

/**
 * 개선 요청 첨부의 stored_path. `improvement-requests/{개선요청id}/{첨부id}.{확장자}`
 *
 * buildAttachmentStoredPath와 같은 규칙을 따른다 — 확장자는 허용목록에서
 * 정규화한 소문자만 붙고, 목록 밖 확장자는 던진다.
 */
export function buildImprovementRequestAttachmentStoredPath(params: {
  ...
  const improvementRequestId = requireUuid("개선 요청 ID", params.improvementRequestId);
  const attachmentId = requireUuid("첨부 ID", params.attachmentId);
  ...
  // 접수 건 쪽과 같은 이유로 path.join을 쓰지 않는다(파일 헤더 규칙 1).
  return `${ATTACHMENT_IMPROVEMENT_REQUEST_STORED_PATH_PREFIX}/${improvementRequestId}/${attachmentId}.${extension}`;
}

/**
 * 개선 요청 첨부의 미리보기(썸네일) 경로. 원본과 같은 폴더에 나란히 두고,
 * 확장자는 언제나 jpg다 — buildAttachmentPreviewPath의 주석과 같은 이유다.
 */
export function buildImprovementRequestAttachmentPreviewPath(params: {
  improvementRequestId: string;
  attachmentId: string;
}): string {
  const improvementRequestId = requireUuid("개선 요청 ID", params.improvementRequestId);
  const attachmentId = requireUuid("첨부 ID", params.attachmentId);
  return `${ATTACHMENT_IMPROVEMENT_REQUEST_STORED_PATH_PREFIX}/${improvementRequestId}/${attachmentId}.preview.jpg`;
}

/**
 * 원본 파일명에서 바로 개선 요청 첨부의 stored_path를 만드는 편의 함수.
 * buildAttachmentStoredPathFromFileName과 짝이며, 확장자를 뽑을 수 없으면 던진다.
 */
export function buildImprovementRequestAttachmentStoredPathFromFileName(params: {
  improvementRequestId: string;
  attachmentId: string;
  originalFileName: string;
}): string {
  ...
  return buildImprovementRequestAttachmentStoredPath({
...
/**
 * ============================================================================
 * 견적서 첨부 — 네 번째 벌 (2026-09-15)
 * ============================================================================
 * 견적서에 붙는 결재 견적서 PDF · 수기 견적서 엑셀의 자리다. 앞의 세 벌과 같은
 * 까닭으로 **건드리지 않고 나란히 새로 둔다** — 서명도 본문도 그대로다.
 *
 * 검사 규칙은 앞의 세 벌과 **글자 하나까지 같다** — UUID 형식 확인, 확장자 허용목록,
...
export function assertPortableStoredPath(storedPath: string): void {
  ...
  // 첫 마디는 **알려진 접두어 중 하나여야 한다**(지금은 넷 — 목록은
  ...
  // 검사하므로, 여기가 모델·개선 요청 경로를 안 받으면 나중에 백업이 그
  // 파일에서 멈춘다.
