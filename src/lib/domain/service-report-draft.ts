 ...
 * ============================================================================
 * 보고서 화면에는 **아직 DB 에 저장하는 표가 없다.** 사람이 본문에 열 줄 스무 줄
 * 적어 둔 뒤 실수로 새로고침하면 통째로 날아간다. 그래서 적는 대로 그 브라우저에
 * 적어 두었다가 다시 들어오면 되살린다.
 *
 ...
 */
export function serviceReportDraftStorageKey(userKey: string, repairCaseId: string): string {
...
/** 지운다(「새로 시작」). 읽기·적기와 같은 이유로 **던지지 않는다.** */
 ...
 */
const DRAFT_SAVED_AT_FORMATTER = new Intl.DateTimeFormat("en-CA", {
  timeZone: "Asia/Seoul",
  year: "numeric",
  month: "2-digit",
  day: "2-digit",
...
/**
 * 안내에 적을 시각 — `2026-09-02 14:33`.
 *
 * 읽을 수 없으면 `null` 이다(모르는 시각을 지어내지 않는다). 그때 안내는 시각
 * 없이 "되살렸습니다"만 말한다.
 *
...
export function formatServiceReportDraftSavedAt(savedAt: string | null): string | null {
  ...
  const parts = DRAFT_SAVED_AT_FORMATTER.formatToParts(at);
