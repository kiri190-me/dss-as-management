 ...
 * 실제로 있는** 사유들뿐이고, 아래 일곱이 그것이다.
 *
 * 잠김(account_locked)과 사용 중지(account_disabled)도 이 규칙을 깨지 않는다.
 * 이 둘을 받는 사람은 포털이 이미 본인임을 확인한 사람이고, 보이는 것은 **자기
 * 계정**의 상태뿐이다 — 남의 이메일을 넣어 떠볼 입구가 없으니 "누가 사용자인지"
 * 조회하는 창구가 되지 않는다(api/auth/sso/callback 의 SPOKEN 주석과 같은 까닭).
 * 삭제된 계정은 포털 로그인으로 되살아난 뒤 이 검사를 받는다(auth/sso-login.ts).
 *
...
const SSO_ERROR_MESSAGES: Record<string, string> = {
  ...
  account_locked: "계정이 잠겨 있습니다. 관리자에게 잠금 해제를 요청하세요.",
  account_disabled: "이 시스템에서 사용이 중지된 계정입니다. 관리자에게 문의하세요.",
};
