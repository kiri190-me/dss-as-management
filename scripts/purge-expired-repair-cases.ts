...
import { runRepairCasePurgeSweep } from "../src/lib/db/mutations/repair-cases-purge";

/**
 * Manual/scheduled entry point for the automatic 15-day repair-case purge.
 * Not wired to any OS scheduler yet (Windows Task Scheduler setup is a
 * separate, later step) — this is the command that scheduler will
 * eventually invoke, and can already be run by hand via `npm run
 * purge:repair-cases`. Never touches page requests or an in-process timer —
 * CLI-only, same design as scripts/purge-expired-flowcharts.ts.
 */
async function main() {
  console.log("Running repair-case purge sweep...");
  ...
  if (summary.errored > 0) {
    console.error("Per-case errors:");
    for (const { repairCaseId, message } of summary.errors) {
      console.error(`  - ${repairCaseId}: ${message}`);
    }
  }
...
main()
  .then(async (summary) => {
    ...
    // Non-zero exit when any individual case errored, even though the
